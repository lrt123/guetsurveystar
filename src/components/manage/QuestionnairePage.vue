<template>
    <div>
                        ...
                        <el-card shadow="hover">
                            ...
                            <div v-if="student">
                                <el-button v-if="item.is_need_fill"
                                           ...
                                           type="primary"
                                           class="form-btn"
                                           icon="el-icon-edit-outline">填写问卷</el-button>
                                <el-button v-if="item.is_need_fill===false"
                                           @click="go_fill(item.id,1)"
                                           class="form-btn"
                                           type="success"
                                           icon="el-icon-tickets">查看详情</el-button>
                            </div>
                            <!--教师按钮-->
                            <div v-if="teacher">
                                <el-button v-if="item.editable"
                                           ...
                                           type="primary"
                                           class="form-btn"
                                           icon="el-icon-edit-outline">编辑问卷</el-button>
                            </div>
                        </el-card>
                    </div>
                </el-timeline-item>
        ...
        <el-dialog
                ...
                width="15%"
        >
            <el-select @change="getLessonNo" v-model="courseNo" placeholder="请选择课程">
            ...
            <el-select style="margin-top: 16px" v-model="lessonNo" placeholder="请选择课号">
                <el-option v-for="(value,index) in lessonNoInfo"
                           :key="index"
                           :label="value"
                           :value="value"
                >
...
export default {
    ...
    data() {
        return {
            ...
            funcBtnData: [
                ...
                {id:'questionnaire-btn-fastcreate', value: '快速创建',type: 'primary', icon: 'el-icon-s-opportunity'}
            ],
            dropDownData: [
                {id:'questionnaire-btn-post', value: '发布', type: 'success', icon: 'el-icon-position'},
                {id:'questionnaire-btn-stop', value: '停止', type: 'warning', icon: 'el-icon-remove-outline'},
    ...
    methods: {
        ...
        onClickFuncMenu(args) {
            if (args.value == '发布' || args.value == '停止') {
                    ...
                    if (data.code === 0) {
                        data.data.my_questionnaire.forEach(item => {
                            ...
                            item.ex = [
                                ...
                                {key: '[课程]：', value: item.lesson_code + '-' + item.lesson_name},
        ...
        //快速创建
        fast_create(){
            if (this.lessonNo === ''||this.courseNo === ''){
                this.$message({message:"请选择课程，课号！",duration: 2000})
                return
            }
        ...
        getLessonNo(){
            this.lessonNo = ''
            this.$http.get("/course/mylessonno/"+this.courseNo,{})
            .then((res)=>{
                if (res.data.code === 0){
                    let data = res.data.data
                    this.lessonNoInfo=data.lesson_no
                }
            })
        }
    }
